...
const { ERRORS, registerError, ConnectionError } = require('web-soft-server');

registerError('SECURITY_PASSWORD_ERROR', 40600, 'Problem with password strength.');
registerError('ACCOUNT_LOCKOUT', 40601, 'Too many fail attempts to login.');
...
class LockoutManager {
  ...
  access(username) {
    if (this.lockouts.has(username)) {
      ...
      if (lockout.isLock() === true) throw new ConnectionError(ERRORS.ACCOUNT_LOCKOUT, { time: lockout.freeTime() });
...
module.exports = { Auth, LockoutManager, LockoutRecord, OBSERVATION_WINDOW, LOCKOUT_THRESHOLD };
